...
export function Modal({ open, onClose, title, children, showHeader = true }: { open: boolean; onClose: () => void; title: string; children: ReactNode; showHeader?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border border-ink-700 bg-ink-850 p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        {showHeader && (
...
export function BottomSheet({ open, onClose, children }: { open: boolean; onClose: () => void; children: ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-t-3xl border-t border-ink-700 bg-ink-850 p-5 pb-8" onClick={(e) => e.stopPropagation()}>
