...
export function NewAccountModal({open,onClose}:{open:boolean;onClose:()=>void}){
 ...
 const[firmId,setFirmId]=useState('');const[programId,setProgramId]=useState('');const[size,setSize]=useState<AccountSize>('25K');const[name,setName]=useState('');const[code,setCode]=useState('');const[saving,setSaving]=useState(false);const[error,setError]=useState('');
 ...
 const save=async()=>{if(!name.trim()||!code.trim()||!firm||!program)return;setSaving(true);setError('');const result=await addAccount({name:name.trim(),code:code.trim(),size:selectedSize,propProgramId:program.id,propFirmName:firm.name,propProgramName:program.name,rulesSnapshot:snapshot});setSaving(false);if(!result.ok){setError(result.error||'Não foi possível cadastrar a conta.');return;}onClose()};
 ...
 return <Modal open={open} onClose={onClose} title="Nova Conta"><div className="space-y-5">
  ...
  <div><label className="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-500">Código Oficial (ID da Conta)</label><input value={code} onChange={e=>setCode(e.target.value.replace(/[^a-zA-Z0-9]/g,''))} inputMode="text" placeholder="Ex: FP10K-A749302" className="input"/></div>
  <div className={`rounded-2xl border ${colors.ring.replace('ring-','border-')}/20 ${colors.soft} p-4`}><div className="flex items-center justify-between"><div><p className={`text-xs font-bold ${colors.text}`}>{firm?.name||'Mesa'} · {program?.name||'Programa'}</p><p className="mt-1 text-[11px] text-gray-500">{program?.phases.length||0} {program?.phases.length===1?'fase':'fases'} · regras vinculadas automaticamente</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${colors.soft} ${colors.text}`}>{colors.label}</span></div><div className="mt-4 grid grid-cols-3 gap-2 text-[10px]">{snapshot[0]&&<><div><span className="text-gray-600">Meta</span><strong className="mt-1 block text-gray-300">{snapshot[0].targetPct!=null?`${snapshot[0].targetPct}%`:'—'}</strong></div><div><span className="text-gray-600">Drawdown</span><strong className="mt-1 block text-gray-300">{snapshot[0].maxDrawdown!=null?`$${snapshot[0].maxDrawdown.toLocaleString('en-US')}`:snapshot[0].drawdownType||'—'}</strong></div><div><span className="text-gray-600">Fase</span><strong className="mt-1 block text-gray-300">1 / {program?.phases.length||1}</strong></div></>}</div></div>
  {error&&<div className="rounded-xl border border-red-500/20 bg-red-500/[0.06] px-3 py-2 text-xs text-red-300">{error}</div>}\n  <button onClick={save} disabled={saving||!name.trim()||!code.trim()||!firm||!program} className="w-full rounded-2xl border border-[#D4AF37]/50 bg-[#D4AF37]/[0.06] py-3.5 font-bold text-[#D4AF37] disabled:cursor-not-allowed disabled:opacity-40">{saving?'Salvando...':'Cadastrar Conta'}</button>
 </div></Modal>;
}
