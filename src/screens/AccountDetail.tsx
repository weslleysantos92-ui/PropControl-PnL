...
const PNL_RULES: Record<string, { capital:number; target:number; drawdown:number; floor:number; daily:number }> = {
  ...
  '50K': { capital:50000, target:3000, drawdown:2500, floor:47500, daily:1250 },
  '100K': { capital:100000, target:6000, drawdown:4700, floor:95300, daily:2350 },
  '150K': { capital:150000, target:9000, drawdown:6000, floor:144000, daily:3000 },
};
...
function PremiumCard({account}:{account:Account}){const funded=account.status==='Financiada';return <section className="relative overflow-hidden rounded-3xl border border-[#5A4816] bg-gradient-to-br from-[#17140B] via-[#0F0F12] to-[#09090A] p-6 shadow-[0_0_35px_rgba(212,175,55,.08)]"><div className="absolute -inset-x-1/2 top-0 h-px bg-gradient-to-r from-transparent via-[#FFE58A] to-transparent animate-pulse"/><div className="relative flex items-center gap-4"><div className="h-14 w-14 rounded-full border border-[#D4AF37]/50 bg-black/30 grid place-items-center text-[#D4AF37]"><Crown size={26}/></div><div className="min-w-0 flex-1"><p className="text-2xl md:text-3xl font-black truncate">{account.name}</p><p className="text-sm text-gray-500 mt-1">{account.propFirm} • {account.size}</p></div><span className={`shrink-0 px-3 py-1.5 rounded-full border text-xs font-bold ${funded?'border-emerald-400/30 bg-emerald-400/10 text-emerald-400':'border-blue-500/30 bg-blue-500/10 text-blue-400'}`}>• {funded?'FUNDED PRO':'CHALLENGE'}</span></div></section>}
...
function RiskPanel({account,trades}:{account:Account;trades:Trade[]}){
  ...
  const consistencyOk=(funded?proConsistency:challengeConsistency)<=(funded?40:50)&&(funded?availableForPayout:1)>0;
  return <section className="rounded-3xl border border-[#27272F] bg-[#111116] p-5 md:p-6">
    ...
    <div className="mt-4 rounded-2xl border border-[#27272F] bg-[#0D0D11] p-4"><div className="flex items-center justify-between gap-3"><span className="text-xs text-gray-400">Consistência · {funded?'40% para saque':'50% Challenge'}</span><span className={`text-xs font-bold ${consistencyOk?'text-emerald-400':'text-amber-400'}`}>{consistencyOk?<><Check size={14} className="inline mr-1"/>Dentro da regra</>:<>● Concentração Alta. Opere mais dias para diluir.</>}</span></div><p className="text-[11px] text-gray-600 mt-2">Maior dia: {formatCurrency(maxDay)} · {funded?'Base disponível para saque: '+formatCurrency(availableForPayout):'Meta: '+formatCurrency(r.target)}</p></div>
  </section>
}
function Info({label,value,icon:Icon}:{label:string;value:string;icon:any}){return <div className="rounded-2xl border border-[#27272F] bg-[#0E0E12] p-3"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-gray-600"><Icon size={14}/>{label}</div><p className="mt-2 text-sm font-bold text-white">{value}</p></div>}
...
function Stat({label,value,cls,icon:Icon}:{label:string;value:string;cls:string;icon:any}){return <div className="rounded-2xl border border-[#27272F] bg-[#121217] p-5"><div className="flex items-center justify-between"><span className="text-[10px] uppercase tracking-wider text-gray-500 font-bold">{label}</span><Icon size={16} className="text-gray-600"/></div><p className={`mt-4 text-xl font-black ${cls}`}>{value}</p></div>}
