...
function RiskPanel({ account, trades }: { account: Account; trades: Trade[] }) {
  ...
  const phase = account.phase === 2 ? 2 : 1;
  const target = phase === 1 ? r.phase1Target : r.phase2Target;
  const drawdown = r.maxLoss;
  ...
  const riskPct = Math.min(100, lossUsed / drawdown * 100);
  ...
  const profitableDays = new Set(trades.filter(t => t.amount >= profitableDayMinimum).map(t => new Date(t.timestamp).toDateString())).size;
