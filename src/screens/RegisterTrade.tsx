...
export function RegisterTrade({ accountId, onBack, onOpenDetails }: RegisterTradeProps) {
  ...
  const displayName = useMemo(() => {
    ...
    return account.name.replace(/lucid/gi, 'PnL Global');
  }, [account]);

  if (!account) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center text-white">
        <button onClick={onBack} className="text-sm text-[#D4AF37]">Conta não encontrada. Voltar</button>
      </div>
    );
  }
    ...
    <div className="register-trade mx-auto w-full max-w-[920px] px-4 pb-28 md:px-6" style={{ fontFamily: 'Manrope, Inter, sans-serif' }}>
      <div className="flex items-start justify-between pt-2 md:pt-4">
        ...
        <div className="text-center pt-0.5">
          <h1 className="text-[28px] font-extrabold tracking-tight text-white md:text-[34px]">Registrar Trade</h1>
          <p className="mt-1 text-sm text-[#A7A7AE] md:text-base">Registre sua execução e continue sua jornada.</p>
        </div>
        <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-transparent text-[#D4AF37]"><FileText size={27} /></div>
      </div>

      <section className="mt-6 rounded-[22px] border border-[#806615] bg-[radial-gradient(circle_at_20%_10%,rgba(212,175,55,.08),transparent_35%),#0D0D10] p-5 shadow-[0_15px_40px_rgba(0,0,0,.35)] md:p-7">
        <div className="flex flex-col gap-5 md:flex-row md:items-center">
          <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-2xl border border-[#6D5410] bg-[#09090B] text-[#D4AF37] shadow-[0_0_25px_rgba(212,175,55,.08)]">
            <span className="text-4xl font-black">P</span>
          </div>
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-[26px] font-extrabold text-white">{displayName}</h2>
            <span className={`mt-2 inline-flex rounded-full px-3 py-1 text-xs font-extrabold uppercase tracking-wider ${account.status === 'Financiada' ? 'bg-green-500/10 text-green-400' : 'bg-blue-500/10 text-blue-400'}`}>
              {account.status === 'Financiada' ? 'FINANCIADA' : 'AVALIAÇÃO'}
            </span>
            <div className="mt-4 flex gap-8 text-sm text-[#A7A7AE]">
              <div><span className="block text-xs text-[#777780]">Mesa</span><strong className="text-lg text-white">PnL Global</strong></div>
      ...
      <section className="mt-5 rounded-[22px] border border-[#38331F] bg-[#101014] p-5 shadow-[0_15px_40px_rgba(0,0,0,.3)] md:p-7">
        ...
        <div className="my-6 border-t border-[#24242A]" />
        <SectionTitle icon={<Trophy size={23} />} title="Resultado" />
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <ResultButton active={result === 'Take'} color="#2ECC71" icon={<Check size={19} />} label="TAKE" onClick={() => setResult('Take')} />
          <ResultButton active={result === 'Stop'} color="#E04B4B" icon={<X size={19} />} label="STOP" onClick={() => setResult('Stop')} />
          <ResultButton active={result === 'BE'} color="#A5A5AC" icon={<Minus size={19} />} label="BE" onClick={() => setResult('BE')} />
        </div>

        <div className="my-6 border-t border-[#24242A]" />
        <label className="block">
          <span className="mb-3 flex items-center gap-3 text-base text-[#C6C6CB]"><DollarSign size={22} className="text-[#D4AF37]" />Resultado Financeiro (USD)</span>
          <div className="flex h-16 items-center rounded-2xl border border-[#303036] bg-[#17171C] px-5 focus-within:border-[#806615]">
            <span className="mr-2 text-xl font-bold text-[#777780]">US$</span>
            <input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" placeholder="0,00" className="min-w-0 flex-1 bg-transparent text-2xl font-bold text-white outline-none placeholder:text-[#55555E]" />
            <span className="rounded-xl border border-[#292930] bg-[#101014] px-4 py-2 text-sm font-bold text-[#D7D7DC]">USD</span>
          </div>
        </label>

        <div className="my-6 border-t border-[#24242A]" />
        <label className="block">
          <span className="mb-3 flex items-center gap-3 text-base text-[#C6C6CB]"><NotebookPen size={22} className="text-[#D4AF37]" />Observação <span className="text-sm text-[#777780]">(opcional)</span></span>
          <div className="relative">
            <textarea value={note} onChange={(e) => setNote(e.target.value.slice(0, 300))} maxLength={300} placeholder="Digite sua observação..." className="min-h-36 w-full resize-none rounded-2xl border border-[#303036] bg-[#17171C] p-5 pb-10 text-base text-white outline-none placeholder:text-[#66666F] focus:border-[#806615]" />
      ...
      <div className="mt-5 flex items-center gap-4 rounded-[22px] border border-[#303036] bg-[#121216] p-5 text-[#B7B7BE]">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#D4AF37]/10 text-[#D4AF37]"><ShieldCheck size={24} /></div>
        <p className="text-sm leading-6 md:text-base">Disciplina é executar seu plano,<br />não importa o resultado.</p>
      </div>
    </div>
  );
}
...
function ResultButton({ active, color, icon, label, onClick }: { active: boolean; color: string; icon: React.ReactNode; label: string; onClick: () => void }) {
