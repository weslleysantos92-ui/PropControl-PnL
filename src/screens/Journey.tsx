    ...
    <div className="px-4 pt-4 pb-28 space-y-5">
      <div className="relative overflow-hidden rounded-3xl border border-size100-ring bg-gradient-to-br from-size100-soft to-ink-850 p-5">
        <div className="relative">
          <div className="flex items-center gap-2 mb-2"><Trophy size={20} className="text-size100-text" /><span className="text-[11px] font-bold text-size100-text uppercase tracking-wider">Grande Objetivo Master</span></div>
          ...
          <div className="flex items-center gap-3"><div className="flex-1 h-3 rounded-full bg-ink-800 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-size100 to-size100-text" style={{ width: `${Math.min(100, (master.current / master.target) * 100)}%` }} /></div><span className="text-sm font-bold text-white">{master.current}/{master.target}</span></div>
          {master.complete && <div className="mt-3 flex items-center gap-1.5 text-funded-text text-sm font-semibold"><Check size={16} /> Objetivo concluído!</div>}
        </div>
      </div>
      {levels.map((level) => {
        const items = achievements.filter(a => a.level === level);
        return <div key={level}><div className="flex items-center gap-2 mb-3"><span className={`w-2.5 h-2.5 rounded-full ${SIZE_COLORS[level].dot}`} /><h3 className="text-sm font-bold text-gray-300">Nível {level}</h3></div><div className="space-y-2">{items.map(a => <AchievementCard key={a.id} achievement={a} level={level} />)}</div></div>;
      })}
    </div>
  );
}
