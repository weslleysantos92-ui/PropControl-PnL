    ...
    <button onClick={onClick} className="w-full text-left relative overflow-hidden rounded-2xl p-5 transition-all duration-200 active:scale-[0.99] animate-dash-card-in" style={{ background: 'linear-gradient(145deg, #191400 0%, #0D0D10 60%, #0A0A0C 100%)', border: '1px solid rgba(212,175,55,0.5)', boxShadow: '0 0 30px rgba(212,175,55,0.1), 0 8px 32px rgba(0,0,0,0.5)' }}>
      ...
      <div className="relative flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[12px] font-bold mt-0.5 uppercase tracking-wider" style={{ color: '#D4AF37' }}>{account.size}&nbsp;•&nbsp;<span style={{ color: accentColor }}>{phaseLabel}</span></div>
...
function EvalCard({ account, index, trades, onClick }: { account: Account; index: number; trades: any[]; onClick: () => void }) {
  ...
  return <button onClick={onClick} className="w-full text-left flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 active:scale-[0.98] animate-dash-card-in" style={{ background: '#17171C', border: '1px solid rgba(59,130,246,0.2)', borderLeft: '3px solid #3B82F6', boxShadow: '0 4px 20px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.03)' }}><div className="w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-[13px] font-extrabold" style={{ background: 'rgba(59,130,246,0.1)', color: '#3B82F6' }}>{index}</div><BarChart2 size={26} style={{ color: '#3B82F6', flexShrink: 0 }} strokeWidth={1.8} /><div className="flex-1 min-w-0"><div className="text-[14px] font-extrabold text-white truncate">{account.name}</div><div className="text-[10px] font-bold uppercase tracking-wider mt-0.5" style={{ color: '#3B82F6' }}>{account.size}&nbsp;•&nbsp;{phaseLabel}</div></div><div className="flex flex-col gap-1 w-[100px] flex-shrink-0"><div className="flex items-center justify-between"><span className="text-[9px] font-bold uppercase tracking-widest leading-none" style={{ color: '#9CA3AF' }}>Meta</span><span className="text-[12px] font-extrabold leading-none" style={{ color: '#3B82F6' }}>{stats.progressPct}%</span></div><div className="w-full h-[5px] rounded-full overflow-hidden" style={{ background: 'rgba(42,42,49,1)' }}><div className="h-full rounded-full animate-dash-bar-fill" style={{ width: `${stats.progressPct}%`, background: 'linear-gradient(90deg, #1d4ed8, #3B82F6)', boxShadow: '0 0 5px rgba(59,130,246,0.6)' }} /></div><span className="text-[9px] leading-none" style={{ color: '#4B5563' }}>Alvo: {targetPct}%</span></div><ChevronRight size={16} style={{ color: '#4B5563' }} strokeWidth={2} /></button>;
}
