  ...
  return (
    <button onClick={onClick} className="w-full text-left relative overflow-hidden rounded-2xl p-5 transition-all duration-200 active:scale-[0.99] animate-dash-card-in" style={{ background: 'linear-gradient(145deg, #191400 0%, #0D0D10 60%, #0A0A0C 100%)', border: '1px solid rgba(212,175,55,0.5)', boxShadow: '0 0 30px rgba(212,175,55,0.1), 0 8px 32px rgba(0,0,0,0.5)' }}>
      ...
      <div className="relative grid grid-cols-2 gap-4 mt-4 pt-4" style={{ borderTop: '1px solid rgba(212,175,55,0.15)' }}>
        {isFunded ? <><div><div className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: '#9CA3AF' }}>Dias Lucrativos</div><div className="flex items-center gap-1.5">{Array.from({ length: stats.profitableDaysTarget }).map((_, i) => <span key={i} className="w-3 h-3 rounded-full" style={{ background: i < stats.profitableDays ? '#2ECC71' : 'rgba(42,42,49,0.9)', boxShadow: i < stats.profitableDays ? '0 0 6px rgba(46,204,113,0.5)' : 'none' }} />)}<span className="text-[13px] font-extrabold ml-1" style={{ color: '#2ECC71' }}>{stats.profitableDays} / {stats.profitableDaysTarget}</span></div></div><div><div className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: '#9CA3AF' }}>Próximo Saque Possível Em</div><div className="flex items-center gap-2"><Calendar size={16} style={{ color: '#D4AF37' }} strokeWidth={2} />{daysLeft === null ? <span className="text-[13px] font-bold" style={{ color: '#9CA3AF' }}>Aguardando 1º trade</span> : daysLeft === 0 ? <span className="text-[18px] font-extrabold" style={{ color: '#2ECC71' }}>Disponível</span> : <span className="text-[18px] font-extrabold" style={{ color: '#F5F5F5' }}>{daysLeft} {daysLeft === 1 ? 'dia' : 'dias'}</span>}</div></div></> : <><div><div className="flex items-center gap-2 mb-2"><Target size={13} style={{ color: stats.progressPct < 0 ? '#EF4444' : '#3B82F6' }} strokeWidth={2.2} /><span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: '#9CA3AF' }}>Progresso da Meta</span></div><div className="flex items-center gap-3"><span className="text-[20px] font-extrabold leading-none" style={{ color: stats.progressPct < 0 ? '#EF4444' : '#3B82F6' }}>{stats.progressPct}%</span><div className="flex-1 h-[6px] rounded-full overflow-hidden" style={{ background: 'rgba(42,42,49,1)' }}><div className="h-full rounded-full" style={{ width: `${Math.min(100, Math.max(0, stats.progressPct))}%`, background: stats.progressPct < 0 ? 'linear-gradient(90deg,#7f1d1d,#EF4444)' : 'linear-gradient(90deg,#1d4ed8,#3B82F6)', boxShadow: stats.progressPct < 0 ? '0 0 6px rgba(239,68,68,0.6)' : '0 0 6px rgba(59,130,246,0.6)' }} /></div></div></div><div><div className="flex items-center gap-2 mb-2"><BarChart2 size={13} style={{ color: '#3B82F6' }} strokeWidth={2.2} /><span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: '#9CA3AF' }}>Meta de Avaliação</span></div><div className="flex items-center gap-2"><span className="text-[18px] font-extrabold" style={{ color: '#3B82F6' }}>{account.phase === 2 ? EVALUATION_TARGET_PCT.phase2 : EVALUATION_TARGET_PCT[account.size]}%</span></div></div></>}
      </div>
    </button>
  );
}
