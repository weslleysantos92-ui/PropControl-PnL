...
export interface AccountRuleSnapshot {
  target?: number; targetPct?: number; drawdownType?: 'Estático' | 'EOD'; maxDrawdown?: number;
  dailyLoss?: number; consistencyEnabled?: boolean; consistencyPct?: number;
  profitableDaysEnabled?: boolean; profitableDays?: number; profitableDayPct?: number;
  cushionEnabled?: boolean; cushion?: number;
}

export interface Account {
  id: string; name: string; code: string; size: AccountSize; status: AccountStatus;
  propFirm?: string; propFirmName?: string; propProgramName?: string; propProgramId?: string;
  phase?: AccountPhase; currentPhase?: number; rulesSnapshot?: AccountRuleSnapshot[];
...
export interface JourneyObjective {
  id: string; name: string; type: JourneyObjectiveType; value?: number; progress: number;
  deadline?: string; icon: string; description?: string; completed: boolean; completedAt?: number;
}
export interface JourneyState { startedAt?: number; unlockedAchievements: string[]; objective?: JourneyObjective | null; }
