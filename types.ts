...
export interface AccountRuleSnapshot { target?: number; targetPct?: number; drawdownType?: 'Estático' | 'EOD'; maxDrawdown?: number; dailyLoss?: number; consistencyEnabled?: boolean; consistencyPct?: number; profitableDaysEnabled?: boolean; profitableDays?: number; profitableDayPct?: number; cushionEnabled?: boolean; cushion?: number; }
export interface Account { id: string; name: string; code: string; size: AccountSize; status: AccountStatus; createdAt: number; queueOrder: number; fundedAt?: number; propFirmName?: string; propProgramName?: string; propProgramId?: string; currentPhase?: number; rulesSnapshot?: AccountRuleSnapshot[]; }
...
export const PNL_RULES: Record<AccountSize,PnLRule> = {'25K':{capital:25000,target:1500,drawdown:1500,floor:23500,daily:750},'50K':{capital:50000,target:3000,drawdown:3000,floor:47000,daily:1500},'100K':{capital:100000,target:6000,drawdown:6000,floor:94000,daily:3000},'150K':{capital:150000,target:9000,drawdown:9000,floor:141000,daily:4500}};
export interface AccountColor { soft:string; text:string; ring:string; dot:string; label:string; }
export const SIZE_COLORS: Record<AccountSize,AccountColor> = {'25K':{soft:'bg-size25-soft',text:'text-size25-text',ring:'ring-size25-ring',dot:'bg-size25',label:'Azul Safira'},'50K':{soft:'bg-size50-soft',text:'text-size50-text',ring:'ring-size50-ring',dot:'bg-size50',label:'Roxo Neon'},'100K':{soft:'bg-size100-soft',text:'text-size100-text',ring:'ring-size100-ring',dot:'bg-size100',label:'Laranja/Dourado'},'150K':{soft:'bg-size150-soft',text:'text-size150-text',ring:'ring-size150-ring',dot:'bg-size150',label:'Verde Esmeralda'}};
